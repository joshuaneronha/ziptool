...
def _group_weighted_mean(df, group_col, val_col, weight_col):
    ...
    tmp_df = pd.DataFrame(
        {group_col: df[group_col], val_col: df[val_col] * df[weight_col],}
    )
...
def _group_weighted_median(df, group_col, val_col, weight_col):
    ...
    cum_weight_high = df.groupby(group_col)[weight_col].cumsum() >= (
        0.5
        * df.groupby(group_col)[weight_col]
        .sum()
        .repeat(df.groupby(group_col)[weight_col].count())
        .values
    )
    ...
    if variables is not None:
        ...
        for key, value in variables.items():
            ...
            mean = _group_weighted_mean(
                ...
                "PUMA",
                key,
                "PERWT" if value["type"] == "individual" else "HHWT",
            )
        ...
        return (
            all_pumas.loc[our_indices]
            .multiply(pumas.values, axis=0)
            .sum()
            .astype(float)
        )
