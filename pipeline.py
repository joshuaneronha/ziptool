...
with open('resources/stateCodeToFips.json') as file:
    codes = json.load(file)
#var_of_interest, null_val, type
def data_by_zip(zips, acs_data, variables):
    '''
    ...
    for zip in zips:
        tracts, state_fip_code = zip_to_tract(zip)

        if sum([x[1] for x in tracts]) == 0:
            raise Exception(zip + ' is not a valid residential zip code!')

        download_data(state_fip_code)
        puma_ratios = tracts_to_puma(tracts, state_fip_code)

        ans = get_acs_data(acs_data, variables, int(state_fip_code), puma_ratios)
        # ans_list.append(round(ans,2))
        ans_dict[zip] = ans
...
def download_data(state_fip_code):
    ...
    if exists('data/' + tract_file + '.shp'):
        ...
        req, _ = urllib.request.urlretrieve('https://www2.census.gov/geo/tiger/TIGER2019/PUMA/' + puma_file + '.zip')
        zipped = zipfile.ZipFile(req, 'r')
        zipped.extractall(path = 'data/')

        req, _ = urllib.request.urlretrieve('https://www2.census.gov/geo/tiger/TIGER2019/TRACT/' + tract_file + '.zip')
        zipped = zipfile.ZipFile(req, 'r')
        zipped.extractall(path = 'data/')
...
def zip_to_tract(zip):
    ...
    try:
        state = hud_crosswalk[hud_crosswalk['zip'] == zip]['usps_zip_pref_state'].iloc[0]
    except:
        raise Exception(zip_str + ' is not a valid ZIP code!')
    return contained.values.tolist(), codes[state]

def tracts_to_puma(tracts, state_fip_code):
    '''
    ...
    if state_fip_code in state_intersections:
        intersection_gdf = state_intersections[state_fip_code]

    else:
        puma = gpd.read_file('data/tl_2019_' + state_fip_code + '_puma10.shp').to_crs(epsg = ALBERS_EPSG_ID)
        tract = gpd.read_file('data/tl_2019_' + state_fip_code + '_tract.shp').to_crs(epsg = ALBERS_EPSG_ID)

        intersection_gdf = gpd.overlay(puma, tract, how = 'intersection', keep_geom_type=False)
        state_intersections[state_fip_code] = intersection_gdf
    ...
    sorted = grouped_gdf.loc[[int(x[0]) for x in tracts]][['PUMACE10','ratios']].explode(column = ['PUMACE10', 'ratios'])
    sorted['ratios'] = sorted['ratios'].apply(lambda x: 1 if x > 0.99 else x)
    out = sorted[sorted['ratios'] > (1 - 0.99)]
    joined = out.join(pd.DataFrame(tracts).astype({0: 'int', 1: 'float32'}).set_index(0))
    joined['weighted_ratios'] = joined['ratios'] * joined[1]
    return joined.groupby('PUMACE10').sum()['weighted_ratios']

#variables, null_val, type,
def get_acs_data(file, variables, state_fip_code, pumas):
    ...
    for entry in variables:
        ...
        for index, i in enumerate(pumas):
            ...
            if var_type == 'household':
                ...
                median = weighted.median(this_puma[variable],this_puma['HHWT'])
                median_list.append(median * i)

                std = np.sqrt((((this_puma[variable] - avg) ** 2) * this_puma['HHWT']).sum() / (((len(this_puma['HHWT']) - 1) / len(this_puma['HHWT'])) * this_puma['HHWT'].sum()))
                ...
                median = weighted.median(this_puma[variable],this_puma['PERWT'])
                median_list.append(median * i)

                std = np.sqrt((((this_puma[variable] - avg) ** 2) * this_puma['PERWT']).sum() / (((len(this_puma['PERWT']) - 1) / len(this_puma['PERWT'])) * this_puma['PERWT'].sum()))
                std_list.append(std * i)

            else:
                raise(Exception('Not a valid type was specified!'))

        outer_dict[variable] = {"mean": round(sum(avg_list),2), "std": round(sum(std_list),2), "median": round(sum(median_list),2)}
...
## nulls aren't getting filtered out!
